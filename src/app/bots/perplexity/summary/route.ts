...
export async function GET() {
  try {
    ...
    // 6) Build a new Excel workbook for download
    ...
    // 7) Return XLSX download
    return new Response(buffer, {
      status: 200,
      headers: {
        'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'Content-Disposition': 'attachment; filename="pivot_summary.xlsx"',
      },
    });
  } catch (err) {
    console.error('❌ summary error:', err);
    const msg = (err as Error).message;
    return new Response(`Error generating XLSX: ${msg}`, {
      status: 500,
